      ...
      <el-col :span="24">
        ...
        >
          ...
          <el-table-column align="center" label="名称">
            ...
            <template slot-scope="scope">{{ scope.row.gg }}</template>
          </el-table-column>
              <el-table-column align="center" label="故障日期">
            <template slot-scope="scope">{{ scope.row.ShiftTime }}</template>
          </el-table-column>
              <el-table-column align="center" label="是否在报修期内">
            <template slot-scope="scope">{{ scope.row.ShiftContent }}</template>
          </el-table-column>
              <el-table-column align="center" label="厂牌">
            <template slot-scope="scope">{{ scope.row.Approver }}</template>
          </el-table-column>
           <el-table-column align="center" label="故障情况">
            <template slot-scope="scope">{{ scope.row.ApprovalResult }}</template>
          </el-table-column>
          <el-table-column align="center" label="报修部门">
            <template slot-scope="scope">{{ scope.row.Remark }}</template>
          </el-table-column>
          <el-table-column align="center" label="报修人">
          ...
          <div class="number">
            <div>总共 <span>4</span> 条记录，每页显示 <span>14</span> 条</div>
        </div>
        <div class="pag-wrap">
            <el-pagination
            background
...
export default {
  data() {
    return {
      ...
      rolesList: [
        {
          HandOverUser: "门_1DX60978",
          Recipient: '门',
          ShiftContent:'2019/07/24',
          ShiftTime:'是',
          Approver:'上海达声',
          ApprovalResult:'损坏',
          Remark:'物业部',
